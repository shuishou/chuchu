...
- (BOOL)storeString:(NSString*)string forKey:(NSString*)key accessibleAttribute:(CFTypeRef)accessibleAttribute {
	if (!string)  {
        ...
        if (result!=0) {
            CZLog(@"Could not store(Delete) string. Error was:%i",(int)result);
        }
        return !result;
#else //OSX
        SecKeychainItemRef item = NULL;
        ...
        if(!string) {
            ...
            if (result!=0) {
                CZLog(@"Could not store(Update) string. Error was:%i",(int)result);
            }
return !result;
        }else{
            ...
            if (result!=0) {
                CZLog(@"Could not store(Add) string. Error was:%i",(int)result);
            }
            return !result;
        }
#else //OSX
