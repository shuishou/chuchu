...
@property (strong,nonatomic)NSURL *finishedRecoderingUrl;
...
@property (nonatomic,strong) UIButton * remindLabel;
...
-(void)initLongPressView{
    ...
    UILongPressGestureRecognizer * LP = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(didLongPress:)];
    
    LP.minimumPressDuration = 0.5;
    
    _longPressView = [UIView new];
    
    _longPressView.frame = CGRectMake(0, 0, DEVICE_SIZE.width, DEVICE_SIZE.height - 100);
    
    _longPressView.backgroundColor = [UIColor clearColor];
    
    [_longPressView addGestureRecognizer:LP];
    
    [self.view insertSubview:_longPressView belowSubview:_maskView];
    
}

-(void)didLongPress:(UILongPressGestureRecognizer*)LP{
    
    NSLog(@"begin");
    
    if (LP.state == UIGestureRecognizerStateBegan) {
        
        _remindLabel.selected = YES;
...
-(void)initRemindLabel{
    ...
    _remindLabel = [UIButton buttonWithType:UIButtonTypeCustom];
    
    _remindLabel.frame = CGRectMake(0, CGRectGetMaxY(_progressBar.frame)+10, DEVICE_SIZE.width, 21);
    
    _remindLabel.userInteractionEnabled = NO;
    
    [_remindLabel setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    
    [_remindLabel setTitle:@"长按视频来拍摄" forState:UIControlStateNormal];
    
    [_remindLabel setTitle:@"长按视频来拍摄" forState:UIControlStateSelected];
    
    [self.view insertSubview:_remindLabel belowSubview:_maskView];
}
...
- (void)initDeleteButton
{
    ...
    center.y = DEVICE_SIZE.height - 50;
...
- (void)initRecordButton
{
    ...
    self.recordButton = [[UIButton alloc] initWithFrame:CGRectMake((DEVICE_SIZE.width - buttonW) / 2.0, /*_progressBar.frame.origin.y + _progressBar.frame.size.height + 10*/DEVICE_SIZE.height - 50, buttonW, buttonW)];
    
    _recordButton.center = CGPointMake(DEVICE_SIZE.width/2, DEVICE_SIZE.height - 50);
...
//        _okButton.enabled = NO;
        ...
        self.finishedRecoderingUrl = outputFileURL;
    }
...
- (void)videoRecorder:(SBVideoRecorder *)videoRecorder didRecordingToOutPutFileAtURL:(NSURL *)outputFileURL duration:(CGFloat)videoDuration recordedVideosTotalDur:(CGFloat)totalDur
{
    ...
    if (videoDuration + totalDur >= MIN_VIDEO_DUR) {
        
        [_remindLabel setTitle:@"松开可完成拍摄" forState:UIControlStateSelected];
...
- (void)videoRecorder:(SBVideoRecorder *)videoRecorder didFinishMergingVideosToOutPutFileAtURL:(NSURL *)outputFileURL
{
    ...
    if([self.delegate respondsToSelector:@selector(returnVideoURL:)])
    {
        [self.delegate returnVideoURL:outputFileURL];
    }
    [self dismissViewControllerAnimated:YES completion:nil];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(_recordButton.frame, touchPoint)) {
        if(0)
        {
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"重新录制,覆盖之前的吗？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
            alertView.tag = 1000;
            [alertView show];
        }
        else
