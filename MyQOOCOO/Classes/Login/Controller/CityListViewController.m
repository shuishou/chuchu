...
@implementation CityListViewController{
    void(^_chooseBlock)(NSString * cityName);
    
}

- (id)initWithChoose:(void (^)(NSString *))chooseBlock
{
    self = [super init];
    if (self) {
        // Custom initialization
        //把外面的block赋值给内部的block,让它随时在哪里都可用
...
//-(void)choose:(void (^)(NSString *))chooseBlock{
//    _chooseBlock = chooseBlock;
//}
- (void)viewDidLoad
{
...
-(void)getCityData
{
    ...
    [self.cities setObject:_arrayHotCity.count==0?(@[]):(_arrayHotCity) forKey:strHot];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *key = [_keys objectAtIndex:indexPath.section];
    self.selectedCityName = [[_cities objectForKey:key] objectAtIndex:indexPath.row];
    
    _chooseBlock(self.selectedCityName);
    //实现点击代理
