...
@interface QCRiZhiVC ()<UITableViewDataSource,UITableViewDelegate>
{
    UITableView *tableV;
}
...
- (void)requestData {
    ...
    NSDictionary * parameters = @{@"timestamp":@(self.timestamp),@"type":@(3)};
...
- (void)requestUserData{
    self.timestamp =[[NSDate date] timeIntervalSince1970]*1000;
    NSDictionary * parameters = @{@"timestamp":@(self.timestamp),@"type":@(3),@"destUid":self.uid};
