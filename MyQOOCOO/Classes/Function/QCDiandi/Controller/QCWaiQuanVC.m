...
@interface QCWaiQuanVC ()<UITableViewDataSource,UITableViewDelegate>
{
    UITableView *tableV;
}
...
- (void)requestData{
    ...
    NSDictionary * parameters = @{@"timestamp":@(self.timestamp),@"type":@(1)};
...
#pragma mark - 个人中心跳转过来的个人点滴列表数据
- (void)requestUserData{
    self.timestamp =[[NSDate date] timeIntervalSince1970]*1000;
    NSDictionary * parameters = @{@"timestamp":@(self.timestamp),@"type":@(1),@"destUid":self.uid};
