...
@interface QCSingleTVC ()<SRRefreshDelegate>
{
    BOOL isFriends;
}
...
@implementation QCSingleTVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"单身";
    ...
    self.tableView.data = [NSMutableArray array];
    // Do any additional setup after loading the view.
}
-(void)loadData{
//    User * user = [[ApplicationContext sharedInstance] getLoginUser];
    [MBProgressHUD showMessage:nil background:NO];
    [NetworkManager requestWithURL:FRIEND_SINGLE parameter:nil success:^(id response) {
...
//    QCFriendCell * cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([QCFriendCell class]) forIndexPath:indexPath];
//    cell.userModel = self.tableView.data[indexPath.row];
//    cell.delegate = self;
...
//        cell.nicknameLabel.backgroundColor=[UIColor redColor];
