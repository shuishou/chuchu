...
@implementation XSPicBrowseView
{
...
- (void)initImgViewDic{
    ...
    _singleTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(closeSelf:)];
...
- (void)setAllImgVContentFromImage:(NSArray *)imgList{
    ...
    if (imgList.count) {
        if ([imgList[0] isKindOfClass:urlCls]) {
            ...
            [vLift sd_setImageWithURL:[NSURL URLWithString:imgList[MRImgLocationLeft]] placeholderImage:nil options:SDWebImageProgressiveDownload completed:nil];
            [vCenter sd_setImageWithURL:[NSURL URLWithString:imgList[MRImgLocationCenter]] placeholderImage:nil options:SDWebImageProgressiveDownload completed:nil];
            [vRight sd_setImageWithURL:[NSURL URLWithString:imgList[MRImgLocationRight]] placeholderImage:nil options:SDWebImageProgressiveDownload completed:nil];
        }
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if (self == scrollView) {
        ...
        if ([self.picBrowsedelegate respondsToSelector:@selector(XSPicBrowseView:currentIndex:)]) {
            [self.picBrowsedelegate XSPicBrowseView:self currentIndex:_curIndex];
        }
...
-(void)closeSelf:(UITapGestureRecognizer *)sender{
    if ([self.picBrowsedelegate respondsToSelector:@selector(XSPicBrowseView:)]) {
        [self.picBrowsedelegate XSPicBrowseView:self];
    }
}
