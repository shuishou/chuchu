...
- (void)initData
{
    ...
    [NetworkManager requestWithURL:GROUP_GETLIST parameter:@{@"type":@(1)} success:^(id response) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (NSDictionary * dic in model.members) {
        ...
        [imag sd_setImageWithURL:[NSURL URLWithString:dic[@"avatarUrl"]] placeholderImage:[UIImage imageNamed:@"ios-template-120"]];
        if (_HFImageArr.count < 4) {
            [_HFImageArr addObject:imag.image];
        }
