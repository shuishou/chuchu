...
- (void)setMessageModel:(MessageModel *)messageModel
{
    ...
    self.headImageView.layer.cornerRadius = HEIGHT(self.headImageView)/2;
    
    UIImage *placeholderImage = [UIImage imageNamed:@"ios-template-120(1)"];
...
+ (NSString *)cellIdentifierForMessageModel:(MessageModel *)model
{
    ...
    switch (model.type) {
        case eMessageBodyType_Text:
        {
            ...
            if ([model.content containsString:@"send_content_from_card_to_show"]) {
             identifier = [identifier stringByAppendingString:@"cardText"];
            }else{
